    ...
    const fetchProjects = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          console.warn('Projects data not available:', result.error || 'Unknown error')
        }
      } catch (error) {
        ...
        {isLoading ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="relative overflow-hidden rounded-2xl shadow-lg border border-gray-100 animate-pulse"
              >
